...
> **Superseded.** This section describes the generated page that shipped
> first. The page was later rebuilt by hand against a separate brief: it keeps
> the tokens, the sections and the lifted hero block, and departs on type,
> layout, the WebGL hero and the two self-hosted fonts. Two statements below
> are now wrong and are left in place rather than edited, because this is the
> spec the first version was built against: the page is no longer generated by
> `gen-site`, which now refuses to overwrite it, and it cannot be published
> from folder `/impeach/site`, which Pages does not offer. See
> `impeach/site/README.md` for what shipped.
