...
- Publishing the landing page. `impeach/site/` is built, self-contained and
  generated by `gen-site`, but a GitHub Pages branch deployment only offers
  `/` or `/docs` as the source folder and `/docs` already holds the upstream
  CLI's documentation, so the frontend spec's `/impeach/site` is not
  selectable. It needs a Pages workflow, a copy at the root, or a separate
  repository, all of which touch files outside `impeach/`.
